...
/**
 * step-research's configuration in one place. index.js and handler.js read from here —
...
export function buildConfig(settings) {
  const cfg = section(settings, "step-research");
  return {
    // Required, no default — a misconfigured port must fail loudly, not silently
    // bind somewhere unexpected.
    port: cfg.num("port"),
    // Required — a step with nowhere to deliver is a dead end, not a default.
    out: cfg.str("out"),
  };
}
