...
test("deliver forwards the envelope to the next hop and mirrors its answer", async () => {
  ...
  const envelope = { id: "abc", text: "hallo", media: [] };

  const { status, body } = await deliver("http://127.0.0.1:5085/pitches", envelope, { fetch });

  assert.equal(status, 202);
  assert.deepEqual(body, { id: "abc" });
  assert.equal(calls.length, 1);
  assert.equal(calls[0].url, "http://127.0.0.1:5085/pitches");
  assert.equal(calls[0].init.method, "POST");
  assert.deepEqual(JSON.parse(calls[0].init.body), envelope, "the envelope is forwarded verbatim");
});
...
test("deliver mirrors a downstream rejection rather than swallowing it", async () => {
  ...
  const { status, body } = await deliver("http://127.0.0.1:5085/pitches", { id: "x" }, { fetch });
